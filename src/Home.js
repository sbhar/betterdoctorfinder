...
export default class Home extends React.Component {
	...
	fetchInsurances() {
		fetch("https://cors-anywhere.herokuapp.com/https://doc-finder-api.herokuapp.com/api/v1/doctor_database/insurances")
			.then(res => res.json())
			.then(json =>
				this.setState({
	...
	fetchDoctors = () => {
		fetch(
			`https://cors-anywhere.herokuapp.com/https://doc-finder-api.herokuapp.com/api/v1/doctor_database/doctors?q=${this.state.keyword}&longitude=${this.state.longitude}&latitude=${this.state.latitude}`
		)
			.then(res => res.json())
			.then(doctors => {
				this.setState({ doctors: doctors.data });
				console.log('long: ' + this.state.longitude + '-' + this.state.latitude);
			})
	...
	searchWithGoogleCoordinates = () => {
		...
		fetch("https://cors-anywhere.herokuapp.com/https://doc-finder-api.herokuapp.com/api/v1/google", options)
			.then(res => res.json())
			.then(json => {
				if (!json.error){
					this.setState(
